 ...
 * >StackOverflow.com</a> and by <a href=
 * "http://www.icesoft.org/wiki/display/ICE/Spring+Security#SpringSecurity-Step4%3AConfigureYourSpringSecurityredirectStrategy"
...
public class JsfRedirectStrategy implements InvalidSessionStrategy {
	...
	public void onInvalidSessionDetected(HttpServletRequest request,
		...
		if (ajaxRedirect) {
			...
			LOG.debug("Session expired due to ajax request, redirecting to '{}'");

			String ajaxRedirectXml = createAjaxRedirectXml(redirectUrl);
			LOG.debug("Ajax partial response to redirect: {}");

			response.setContentType("text/xml");
			response.getWriter().write(ajaxRedirectXml);
		} else {
			String requestURI = getRequestUrl(request);
			LOG.debug("Session expired due to non-ajax request, starting a new session and redirect to requested url '{}'");
