...
ALTER TABLE `ciudad` CHANGE `departamento_id` `provincia_id` BIGINT(20) NULL DEFAULT NULL;
ALTER TABLE `financiera_dev`.`ciudad` DROP INDEX `FKev870hgt8a7wx5t21q4mnsu3k`, ADD INDEX `provincia_id_fk` (`provincia_id`) USING BTREE;
...
ALTER TABLE `cliente` ADD `fotocopia_dni`  BIT(1) NOT NULL AFTER `garantia`, ADD `recibo`  BIT(1) NOT NULL AFTER `fotocopia_dni`, ADD `servicio`  BIT(1) NOT NULL AFTER `recibo`;